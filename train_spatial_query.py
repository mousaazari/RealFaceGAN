...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, tb_writer, exp_name):
    ...
    sample_z = prepare_noise_new(args.n_sample, args, device, method="query")
    sample_param = prepare_param(args.n_sample, args, device, method="spatial")
    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            ...
            tb_writer.add_scalars("Spatial Path", {'spatial path': spatial_path_loss_val,
                                                   'spatial mean path': mean_spatial_path_length_avg}, i)
