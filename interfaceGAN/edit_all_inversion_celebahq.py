...
sys.path.append("our_interfaceGAN/")
sys.path.append("/")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--z_latent', type=str, default='./projection/encoder_inversion/celebahq_encode/encoded_z.npy')
    parser.add_argument('--p_latent', type=str, default='./projection/encoder_inversion/celebahq_encode/encoded_p.npy')
    ...
    parser.add_argument('--seed', type=int, default=0) # 0:encoder 1:优化
    ...
    ages = []
    genders = []
    attr_classifier = dex.eval(args.attribute_name)
    ...
    if not os.path.exists(z_boundrary_file) or not os.path.exists(p_boundrary_file) or args.retrain_boundrary:
        ...
        with torch.no_grad():
            for b in tqdm(range(num_batch)):
                ...
                # estimate age
                age = dex.estimate_score(attr_classifier, image)
                ages.append(age.cpu())
    ...
    z_latent_projected = np.reshape(z_latent_projected.transpose(0, 2, 1), (count, -1))
    p_latent_projected = np.reshape(p_latent_projected.transpose(0, 2, 1), (count, -1))
    ...
    for e_s in step_dict['style_end_distance']:
        for e_c in step_dict['content_end_distance']:
            ...
            with torch.no_grad():
                w_store_path = os.path.join(args.output_dir,  args.attribute_name, f"{args.seed}")
                # w_store_path = os.path.join(args.output_dir, f"{e_s}_{e_c}", args.attribute_name)
...
# python our_interfaceGAN/edit_all_inversion_celebahq.py --ckpt ./out/trans_spatial_squery_fixed_celeb/checkpoint/370000.pt --attribute_index 0 --z_latent ./projection/encoder_inversion/celebahq_encode/encoded_z.npy --p_latent ./projection/encoder_inversion/celebahq_encode/encoded_p.npy
