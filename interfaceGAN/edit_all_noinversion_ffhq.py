    ...
    for seed in range(seed_dict['seed'][0], seed_dict['seed'][1]):
        for e_s in step_dict['style_end_distance']:
            for e_c in step_dict['content_end_distance']:
                ...
                sample_img_path = os.path.join(args.output_dir, f"{args.attribute_name}/{e_s}_{e_c}_{seed}/origin_image")

                os.makedirs(sample_img_path,  exist_ok=True)

                utils.save_image(
                        img_gen,
                        os.path.join(sample_img_path, "sample_gen.png"),
                        nrow=int(1),
                        normalize=True,
                        range=(-1, 1),
                        padding=0
...
# python our_interfaceGAN/edit_all_noinversion_ffhq.py --ckpt ./out/trans_spatial_squery_multimap_fixed/checkpoint/790000.pt --num_region 1 --num_trans 8 --pixel_norm_op_dim 1 --attribute_name pose