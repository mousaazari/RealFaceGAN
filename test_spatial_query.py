...
def interpolate_style_many(args, sample_path, g_ema, num_tests=10):
    ...
    for j in range(num_tests):
        para_base = prepare_param(10, args, device, method='spatial_same', truncation=args.truncation)
        z_base = prepare_noise_new(8, args, device, "query", truncation=args.truncation)

        boundary_z = torch.randn(1, args.latent)
        results_z = []
        if args.interp_space == "z":
            z_base = z_base.transpose(1, 2)
        if args.interp_space == "z+":
            z_plus = g_ema(z_base, para_base[0].repeat(8, 1, 1), return_only_mapped_z=True).transpose(1,
                                                                                                      2)
            z_base = z_plus.clone()
        elif args.interp_space == "w":
            z_w = g_ema(z_base, para_base[0].repeat(8, 1, 1), return_only_style_latent=True)
            z_base = z_w.clone()

        for i in range(8):
            interpolated_latent = linear_interpolate(z_base[i:i + 1].cpu().numpy(), boundary_z.cpu().numpy(),
...
def interpolate_style_dat(args, sample_path, g_ema, num_tests=10):
    ...
    if not os.path.exists(sample_path):
        os.makedirs(sample_path)
    for j in range(num_tests):
        para_base = prepare_param(6, args, device, method='spatial', truncation=args.truncation)
        z_base1 = prepare_noise_new(6, args, device, "query_same", truncation=args.truncation)
        z_base2 = prepare_noise_new(6, args, device, "query_same", truncation=args.truncation)

        results_z = []

        if args.interp_space == "z+":
            z_plus1 = g_ema(z_base1, para_base, return_only_mapped_z=True)
            z_plus2 = g_ema(z_base2, para_base, return_only_mapped_z=True)
...
def interpolate_spatial_many(args, sample_path, g_ema, num_tests=10):
    ...
    if not os.path.exists(sample_path):
        os.makedirs(sample_path)
    for j in range(num_tests):
        para_base = prepare_param(8, args, device, method='spatial', truncation=args.truncation)
        z_base = prepare_noise_new(10, args, device, "query_same", truncation=args.truncation)

        boundary_para = torch.randn(1, args.latent)

        results_para = []
        if args.interp_space == "p":
            para_base = para_base.transpose(1, 2)
        if args.interp_space == "p+":
            p_plus = g_ema(z_base[0].repeat(8, 1, 1), para_base, return_only_mapped_p=True).transpose(1,
                                                                                                      2)
            para_base = p_plus.clone()

        for i in range(8):
            interpolated_latent = linear_interpolate(para_base[i:i + 1].cpu().numpy(), boundary_para.cpu().numpy(),
                                                     start_distance=-1, end_distance=1)
            if args.interp_space == "p":
                sample, _, _ = g_ema(z_base, torch.from_numpy(interpolated_latent).transpose(1, 2).cuda())
            elif args.interp_space == "p+":
                sample, _, _ = g_ema(z_base, torch.from_numpy(interpolated_latent).transpose(1, 2).cuda(),
                                     use_spatial_mapping=False)
...
def interpolate_spatial_dat(args, sample_path, g_ema, num_tests=10):
    ...
    for j in range(num_tests):
        z_base = prepare_noise_new(6, args, device, "query", truncation=args.truncation)
        para_base1 = prepare_param(6, args, device, method='spatial_same', truncation=args.truncation)
        para_base2 = prepare_param(6, args, device, method='spatial_same', truncation=args.truncation)
