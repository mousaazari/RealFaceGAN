...
class Generator(nn.Module):
    ...
    def interaction_network(self):
        layers = [
            AttentionBlock(in_dim=self.style_dim + 16, param_dim=self.param_dim + 16, out_dim=self.style_dim,
                           lr_mul=self.lr_mlp, )
        ]
        for i in range(1, self.n_trans):
            layers.append(
                AttentionBlock(in_dim=self.style_dim, param_dim=self.param_dim, out_dim=self.style_dim,
                               lr_mul=self.lr_mlp, )
            )
        return nn.Sequential(*layers)
    ...
    def forward(
        ...
        stylecode = stylecode.permute(0, 2, 1)
        spatialcode = spatialcode.permute(0, 2, 1)
        if trans_interact:
            similarity_list = []
            ...
            x, similarity = self.interact[0](input_stylecode, input_spatialcode,
        ...
        if self.no_trans:
            latent = self.adjust_style(stylecode.permute(0, 2, 1)).permute(0, 2, 1)
        else:
            if not input_is_latent:
                latent = self.adjust_style(x.permute(0, 2, 1)).permute(0, 2, 1)
            else:
                latent = style
...
class Attention(nn.Module):
    ...
    def forward(self, attention, op_param, return_similarity=False):

        N, L, C = attention.shape
        _, M, _ = op_param.shape

        q = self.q_transform(op_param).reshape(N, M, self.groups, self.group_planes).permute(0, 2, 3, 1)
        k = self.k_transform(attention).reshape(N, L, self.groups, self.group_planes).permute(0, 2, 3, 1)
        v = self.v_transform(attention).reshape(N, L, self.groups, self.group_planes).permute(0, 2, 3, 1)
        qk = torch.einsum('abcd,abce->abde', q, k) * self.scale
        similarity = F.softmax(qk, dim=3)
        sv = torch.einsum('abcd,abed->abec', similarity, v)
        stacked_output = sv.reshape(N, self.planes, L).permute(0, 2, 1)
